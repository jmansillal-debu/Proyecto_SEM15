...
function initHistory() {
  if (isFirebaseConnected && db) {
    db.ref('history/winners').on('value', snapshot => {
      ...
      if (snapshot.exists()) {
        Object.values(snapshot.val()).reverse().forEach(item => {
          list.innerHTML += `<li class="text-left">🏆 <b>${item.name}</b> [${item.date}]</li>`;
        });
      } else { list.innerHTML = '<li class="empty-msg text-left">Sin campeones registrados aún.</li>'; }
    });
    ...
    db.ref('history/players').on('value', snapshot => {
      ...
      if (snapshot.exists()) {
        Object.values(snapshot.val()).reverse().forEach(item => {
          list.innerHTML += `<li class="text-left">👤 <b>${item.name}</b> (${item.skill})</li>`;
        });
      } else { list.innerHTML = '<li class="empty-msg text-left">Sin jugadores registrados.</li>'; }
    });
  }
}
...
function selectAvatar(element, url, skillName, skillDesc) {
  ...
  document.getElementById('skill-description').innerHTML = '<b>Habilidad:</b> ' + skillDesc;
}
  ...
  opponents.forEach((player, index) => {
    ...
    oppEl.innerHTML = `
      ...
      <div class="text-left" style="width:100%;">
        <div class="opp-name" style="font-size:0.85rem; font-weight:bold;">${player.name}</div>
...
function handleLlamaClick() {
  ...
  if (llamaClicks >= 40) {
    ...
    if (hand.length === 1) {
      alert('⚠️ ¡ABUSO DE MASCOTA DETECTADO! Recibes un castigo de 20 cartas por intentar hacer trampa.');
      let deck = [...currentGameState.deck];
      for (let i = 0; i < 20; i++) {
        if (deck.length === 0) deck = generateDeck();
        hand.push(deck.pop());
      ...
      hand.push({ color: 'negro', value: '+4', id: Math.random().toString(36).substr(2, 9) });
      showToastNotification('🦙 ¡La Llama te dio una carta +4!');
    }
...
function handleGoodPetClick() {
  ...
  if (loroClicks >= 40) {
    ...
    if (hand.length <= 2) {
      showToastNotification('🦜 El Loro no te ayudará a ganar la partida. ¡Descarta tu última carta jugando!');
      return;
    }

    hand.pop();
    showToastNotification('🦜 ¡El Loro Bueno se llevó una de tus cartas!');
...
function useSpecialSkill() {
  if (hasUsedSkill) return alert('Ya has usado tu habilidad única en esta partida.');
  if (currentGameState?.turnOrder[currentGameState.currentTurnIndex] !== myPlayerId) {
    return alert('Solo puedes usar tu habilidad durante tu turno.');
  }
  ...
  if (selectedSkill === 'Escudo Táctico') {
    currentGameState.stack = 0;
    showToastNotification('🛡️ ¡Escudo activado! El pozo de acumulaciones fue anulado.');
  } 
  else if (selectedSkill === 'Espionaje') {
    let opponentInfo = Object.values(currentGameState.players)
      .filter(p => p.id !== myPlayerId)
      .map(p => `${p.name}: ${p.hand.length} cartas (${p.hand.map(c => c.color + ' ' + c.value).join(', ')})`)
      .join('\n\n');
    alert('🔍 Cartas del Rival:\n\n' + opponentInfo);
  } 
  else if (selectedSkill === 'Robo Rápido') {
    currentGameState.stack = (currentGameState.stack || 0) + 4;
    showToastNotification('⚡ ¡Suma+ activado! +4 agregados al pozo de mesa.');
  } 
  else if (selectedSkill === 'Purga Directa') {
    if (hand.length <= 2) {
      return alert('Necesitas tener más de 2 cartas para ejecutar la Purga.');
    }
    hand.splice(0, 2);
    currentGameState.players[myPlayerId].hand = hand;
    showToastNotification('🔥 ¡Purga activada! Descartaste 2 cartas automáticamente.');
  }
